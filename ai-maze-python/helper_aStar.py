...
def compute_node_cost(pos, goal, cost_actual=0):
    """
    Parameters
    ----------
    pos : tuple of 2 ints
    ...
    x, y = pos
    x_goal, y_goal = goal
    # cost = np.sqrt((x_goal-x)**2 + (y_goal-y)**2) # euclidean distance
    # cost = np.abs(x_goal-x) + np.abs(y_goal-y) # manhattan distance
    # euclidean distance + cost for the actual position
    cost = cost_actual + np.sqrt((x_goal-x)**2 + (y_goal-y)**2)
    # manhattan distance + cost for the actual position
    # cost = cost_actual + np.abs(x_goal-x) + np.abs(y_goal-y)
    return cost
...
def compute_successors(current_node, grid, seen):
    ...
    for movement in movements:
        ...
        if cond1:
            ...
            if bool(cond2*cond3):
                cost = compute_node_cost(next_pos, grid_dim, current_node.cost)
